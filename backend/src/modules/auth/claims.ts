import { rolesCliente, rolesRealm, type RoleSicef } from '@sicef/contracts';
import { AppError } from '../../shared/errors.js';

export interface SicefClaims {
  sub: string;
  roles: RoleSicef[];
}
...
type TokenPayload = {
  sub?: unknown;
  resource_access?: { sicef?: { roles?: unknown } };
  realm_access?: { roles?: unknown };
};

// Conserva sólo los roles permitidos para la fuente indicada. Así un `ti`
// colocado por error en `resource_access.sicef.roles` (o un `ventanilla` en
// `realm_access.roles`) no otorga permisos.
...
export function resolveSicefClaims(payload: TokenPayload): SicefClaims {
  if (typeof payload.sub !== 'string' || payload.sub.trim() === '') {
    throw new AppError(401, 'INVALID_TOKEN', 'El token no contiene un sub válido');
  }
  const roles = [
    ...rolesDesde(payload.resource_access?.sicef?.roles, rolesCliente),
