...
import { resolveSicefClaims, type SicefClaims } from './claims.js';

declare global {
  namespace Express {
    interface Request { auth?: SicefClaims; actorId?: string; }
  }
}
...
export function createAuthenticate(env: Env) {
  ...
  return async (request: Request, _response: Response, next: NextFunction) => {
    try {
      ...
      request.auth = resolveSicefClaims(payload);
      next();
    } catch (error) {
...
export function requireRoles(...allowed: SicefClaims['roles']) {
