...
import { crearTramiteSchema, listarTramitesSchema } from '@sicef/contracts';
...
export function createTramitesRouter(internal: RequestHandler[], env: Env): Router {
  ...
  router.get('/:id', async (request, response, next) => {
    ...
    try { const data = await prisma.tramite.findUnique({ where: { id: routeParam(request.params.id, 'id') }, include: { personas: { include: { persona: true } }, evidencias: true, validaciones: true, confirmaciones: true, cobro: true, constancia: true } }); if (!data) throw new AppError(404, 'NOT_FOUND', 'Trámite no encontrado'); response.json({ data: { ...data, constancia: data.constancia ? { ...data.constancia, urlVerificacion: verificador.urlVerificacion(data.constancia.folioUnico, data.constancia.versionToken) } : null }, requestId: request.id }); } catch (error) { next(error); }
  });
