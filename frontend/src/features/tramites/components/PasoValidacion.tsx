...
export function PasoValidacion({ tramite }: { tramite: TramiteDetalle }) {
  ...
  const validacionInicial = tramite.validaciones.find((v) => v.momento === 'VALIDACION_INICIAL');
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.25 }}>
      ...
      {tramite.tipoConstancia === 'NO_ADEUDO' ? (
        ...
        <Alert severity="info" icon={<MsIcon name="map" size={20} />}>
          Constancia de No Registro: la validación es documental (evidencia geográfica y búsqueda en padrón). Si el
          expediente está completo, procede la aprobación.
        </Alert>
      )}
    </Box>
  );
